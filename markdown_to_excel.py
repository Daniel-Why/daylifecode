...
def main():
    ...
    for i in file_route_list:
        ...
        write_excel(new_worksheet,tag_list,sort_by_column=False)
        write_excel(new_worksheet,between_tag_text_list,start_row=n+1,start_column=0,sort_by_column=False)
        n += 1
...
#    tool_name = (lines[0])[2:]
#    #print(lines)
#    new_worksheet.write(n+1,0,tool_name)
#
#    tag_list = []
#    for i in lines:
#        if '###' in i:
...
#
#    for a in tag_list:
#        if t_pointer <= len(tag_list)-2:
#            l_s = tag_list[t_pointer]
#            l_e = tag_list[t_pointer+1]
#        elif t_pointer == len(tag_list)-1:
#            l_s = tag_list[t_pointer]
#            l_e = None
#        text_list = []
#        for i in lines[l_s+1:l_e]:
...
#        md_text = ''.join(text_list)
#        #print(md_text)
#        new_worksheet.write(n+1,t_pointer+1,md_text)
#        t_pointer += 1
#
#    # 保存
#    new_workbook.save(r'%s'%(excel_path))
